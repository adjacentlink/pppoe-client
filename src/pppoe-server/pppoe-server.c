...
#ifdef SUPPORT_RFC4938
/* Use flow control as specified in RFC4938? */
static int EnableRfc4938FlowControl = 0;

/* Use flow control as specified in RFC4938? */
static int AvoidInBandRfc4938FlowControl = 0;

/* The port to use to forward flow control packets to the client */
static int Rfc4938FlowControlPort = DEFAULT_FLOW_CONTROL_PORT;

/* This is the coordination socket used to communicate with the children */
static int rfc4938Socket = -1;

/* The event handler to handle events on the coordination socket */
static EventHandler *rfc4938EventHandler = NULL;
#endif
...
/* Do we enable adding the PPPoE sessions to the CVMI?
 *  - CVMI must have an IPv4 and IPv6 link local address bound to it */
int EnableCVMI = 0;
...
#ifdef SUPPORT_RFC4938
static PPPoETag requestedFlowControl;
#endif
              ...
              void *extra __attribute__((unused)))
{
    ...
    case TAG_CREDITS:
        ...
        // TODO: Else do not accept the session; send an error PADS?
        break;
#endif
    }
}
...
processPADR(Interface *ethif, PPPoEPacket *packet, int len __attribute__((unused)))
{
    ...
    if (EnableRfc4938FlowControl && requestedFlowControl.type)
    {
        ...
        /* copy the FCN field to the BCN field */
        requestedFlowControl.payload[2] = requestedFlowControl.payload[0];
        requestedFlowControl.payload[3] = requestedFlowControl.payload[1];
...
#ifdef SUPPORT_RFC4938
/**********************************************************************
*%FUNCTION: forwardRfc4938Packet
*%ARGUMENTS:
* ethif -- interface
* packet -- PPPoE PADC/PADG/PADQ packet
* len -- length of received packet
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Forward the packet on to the relavent child, since the child can't receive
* this packet themselves
***********************************************************************/
void
forwardRfc4938Packet(Interface *ethif, PPPoEPacket *packet, int len)
{
    ...
    address.sin_port        = htons(Sessions[i].port);
...
usage(char const *argv0)
{
    ...
    fprintf(stderr, "   -U port        -- Change the RFC4938 flow control coordination port.\n");
    fprintf(stderr, "   -a             -- Do not perform RFC4938 flow control in band.\n");
    fprintf(stderr, "   -e             -- Enable binding of PPP session to a CVMI\n");
#endif
...
main(int argc, char **argv)
{
    ...
    else
    {
        ...
        address.sin_family      = AF_INET;
        address.sin_port        = htons(Rfc4938FlowControlPort);
        address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
        ...
        rfc4938EventHandler = Event_AddHandler(event_selector,
                                               rfc4938Socket,
                                               EVENT_FLAG_READABLE,
                                               CoordinationHandler,
                                               NULL);
...
serverProcessPacket(Interface *i)
{
    ...
    case CODE_PADQ:
        ...
        forwardRfc4938Packet(i, &packet, len);
        break;
#endif
    default:
...
startPPPDUserMode(ClientSession *session)
{
    ...
    int c = 0;
    struct in_addr cvmiIp;
    struct in6_addr cvmiIp6;
    int flag = 0;
