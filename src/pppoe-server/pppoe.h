...
* LIC: GPL
*
* $Id: pppoe.h,v 1.5 2008/09/12 13:37:16 powellj Exp $
*
***********************************************************************/
...
/* CVMICTL Path to binary */
#define CVMICTL_BIN "/usr/local/sbin/cvmictl"
...
/* Extensions from RFC4938 (Extensions for Credit Flow and Link Metrics)*/
...
#ifdef SUPPORT_RFC4938
/* Extensions from RFC4938 (Extensions for Credit Flow and Link Metrics)*/
#define TAG_CREDITS            0x106
#define TAG_METRICS            0x107
#define TAG_SEQUENCE_NUMBER    0x108

#define TAG_CREDITS_SIZE          4
#define TAG_METRICS_SIZE         10
#define TAG_SEQUENCE_NUMBER_SIZE  2
#endif
...
#ifdef SUPPORT_RFC4938
#define RFC_MIN_CREDITS       0x0000
#define RFC_PEER_CREDITS      0x8000
#define RFC_MAX_CREDITS       0xffff
#define RFC_PEER_CREDITS_THRESH       0x6d60
#define RFC_BYTES_PER_CREDIT  0x0040
#endif
...
#ifdef SUPPORT_RFC4938
/* The port to use to comunicate with children */
#define DEFAULT_FLOW_CONTROL_PORT 743
#endif
...
typedef struct PPPoEConnectionStruct
{
    ...
    int metricsRequested; /* did the peer request our metrics */
    int enableRfc4938FlowControl; /* should flow control be used */
    int avoidInBandFlowControl; /* shoud in band flow control be avaoided? */
    int initialCredits; /* how many credits to start with? */
    int rfc4938ServerPort; /* What port should out of band flow control be sent to */
    int rfc4938CoordPort; /* What port should bound to for out of band flow control */
    int rfc4938Socket;  /* The socket used to communicate with the server process */
    int padgSequence;   /* Our current sequence number for PADGs */
    int hostCredits;    /* How many credits are available to send packets */
    int peerCredits;    /* How many credits are available to recv packets */
    PPPoEPacket *held;  /* The next packet to send when credits are available */
    int padcPending;    /* is a PADC pending */
#endif
...
void initPPP(void);
#else
void initPPP(PPPoEConnection *conn);
#endif
...
#define SET_SHORT(x, y) do { (x)[0] = (unsigned char) ((y >> 8) & 0xFF);\
                             (x)[1] = (unsigned char) (y & 0xFF); } while(0)
#define GET_FCN(x)    GET_SHORT(x)
#define SET_FCN(x, y) SET_SHORT(x, y)
#define GET_BCN(x)    GET_SHORT((x) + 2)
#define SET_BCN(x, y) SET_SHORT((x) + 2, y)
#define GET_SEQUENCE_NUMBER(x)    GET_SHORT(x)
#define SET_SEQUENCE_NUMBER(x, y) SET_SHORT(x, y)
#endif
