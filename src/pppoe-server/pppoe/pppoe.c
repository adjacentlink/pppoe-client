...
#ifdef SUPPORT_RFC4938
/**********************************************************************
*%FUNCTION: parseRFC4938Tags
*%ARGUMENTS:
* type -- tag type
* len -- tag length
* data -- tag data
* extra -- extra user data, the connection that this packet was received on.
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Picks interesting tags out of a PADG, PADC, or PADQ packet
***********************************************************************/
void
parseRFC4938Tags(UINT16_t type, UINT16_t len, unsigned char *data,
  void *extra) {
    ...
    case TAG_SEQUENCE_NUMBER:
        conn->sequence = GET_SEQUENCE_NUMBER(data);
        break;
    case TAG_METRICS:
        conn->metricsRequested = !len;
        break;
    }
}

/**********************************************************************
*%FUNCTION: modifyForResponse
*%ARGUMENTS:
* packet -- a pointer to a PPPoEPacket
* code -- the new code for the packet
*%RETURNS:
* Nothing
*%DESCRIPTION:
...
modifyForResponse(PPPoEPacket *packet, int code) {
    ...
    memcpy(&buffer[0],              packet->ethHdr.h_dest,   ETH_ALEN);
    memcpy(packet->ethHdr.h_dest,   packet->ethHdr.h_source, ETH_ALEN);
    memcpy(packet->ethHdr.h_source, &buffer[0],              ETH_ALEN);

    /* set the new code for the packet */
    packet->code = ((unsigned char) (code & 0xFF));
}
...
sendPADG (PPPoEConnection *conn, UINT16_t credits)
{
    ...
    cursor += TAG_SEQUENCE_NUMBER_SIZE + TAG_HDR_SIZE;

    /* Add credit Tag */
    creditTag.type = htons(TAG_CREDITS);
    creditTag.length = htons(TAG_CREDITS_SIZE);
    SET_FCN(creditTag.payload, credits);
    SET_BCN(creditTag.payload, conn->hostCredits);

    plen += TAG_CREDITS_SIZE + TAG_HDR_SIZE;
    memcpy(cursor, &creditTag, TAG_CREDITS_SIZE + TAG_HDR_SIZE);
    cursor += TAG_CREDITS_SIZE + TAG_HDR_SIZE;
    ...
    sendPacket(conn, conn->sessionSocket, &packet, (int) (plen + HDR_SIZE));
...
/**********************************************************************
*%FUNCTION: processRfc4938Packet
*%ARGUMENTS:
* conn -- PPPoE connection info
*%RETURNS:
* Nothing
*%DESCRIPTION:
* Handles the additional PPPoE packets specified in RFC 4938
***********************************************************************/
void
processRfc4938Packet(PPPoEConnection *conn)
{
    int size, plen;
    PPPoEPacket packet;
	UINT16_t credits;
    ...
    if (packet.code == CODE_PADG) {
       ...
       LOGGER(LOG_INFO, "pppoe(%u): Recv PADG packet with fcn:0x%04x bcn:0x%04x",
        ...
        /*  respond with a PADC */
        modifyForResponse(&packet, CODE_PADC);
        SET_FCN(conn->creditPtr, RFC_PEER_CREDITS);
        ...
        unsigned short oldHostCredits = conn->hostCredits;
        ...
        if (conn->held) {
            ...
            if (conn->hostCredits - creditCost >= RFC_MIN_CREDITS) {
                ...
                if(!conn->avoidInBandFlowControl) {
                    ...
                    SET_BCN(credits->payload, conn->hostCredits);
                }

                /* send the delayed packet */
                sendSessionPacket(conn, conn->held, conn->held->length);
                conn->held = NULL; /* ready to send more packets from pppd */
            }
        }
#endif
...
usage(char const *argv0)
{
    ...
    fprintf(stderr, "   -x c(:s,p)     -- Start with 'c' credits and coordinate with 's' on 'p'.\n");
...
#ifdef SUPPORT_RFC4938
    conn.initialCredits = -1;
    conn.rfc4938Socket = -1;
  ...
  case 'a':
    ...
    if (conn.enableRfc4938FlowControl) {
        ...
        if (conn.rfc4938CoordPort > 0 && conn.rfc4938ServerPort > 0) {
            ...
            address.sin_family      = AF_INET;
            address.sin_port        = htons(conn.rfc4938CoordPort);
            address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
...
#else
    fcs = pppFCS16(fcs, &packet.payload[0] + offset, plen - offset) ^ 0xffff;
#endif
    tail[0] = fcs & 0x00ff;
    tail[1] = (fcs >> 8) & 0x00ff;
...
#else
    vec[1].iov_base = (void *) (&packet.payload[0] + offset);
#endif
    vec[1].iov_len = plen;
