...
//static char const RCSID[] = "$Id: discovery.c 3 2009-02-04 21:03:29Z bcheng $";
              ...
              void *extra)
{
    ...
    case TAG_CREDITS:
        ...
        // TODO: Else do not accept the session; send a PADT or ignore?
        break;
#endif
    }
}
...
sendPADR(PPPoEConnection *conn)
{
    ...
    {
        PPPoETag credits;
        CHECK_ROOM(cursor, packet.payload, TAG_CREDITS_SIZE + TAG_HDR_SIZE);
        credits.type = htons(TAG_CREDITS);
        credits.length = htons(TAG_CREDITS_SIZE);
        SET_FCN(credits.payload, RFC_PEER_CREDITS);
        SET_BCN(credits.payload, 0);
        conn->peerCredits = RFC_PEER_CREDITS;
        memcpy(cursor, &credits, TAG_CREDITS_SIZE + TAG_HDR_SIZE);
