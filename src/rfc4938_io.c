...
rfc4938_io_forward_to_child (UINT32_t neighbor_id, const void *p2buffer, int buflen, rfc4938_neighbor_element_t *nbr)
{
  ...
  RFC4938_DEBUG_PACKET ("%s:(%u): forward msg from nbr %u, to child on port %hu, len %d\n", 
                        __func__, rfc4938_config_get_node_id (), neighbor_id, nbr->child_port, buflen);

  retval = rfc4938_io_send_to_child (nbr->child_port, p2buffer, buflen);

  if(retval != buflen)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): failed to forward msg from nbr %u, to child on port %hu, len %d\n", 
                            __func__, rfc4938_config_get_node_id (), neighbor_id, nbr->child_port, buflen);
    }
}
...
rfc4938_io_send_to_child (UINT16_t port, const void *p2buffer, int buflen)
{
  ...
  if (z < 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error sending message to %s:%hu\n", __func__,
...
rfc4938_io_send_to_nbr (UINT32_t neighbor_id, UINT16_t credits, const void *p2buffer, int buflen)
{
  ...
  RFC4938_DEBUG_PACKET ("%s:(%u): send message to nbr %u, len %d\n", __func__,
...
rfc4938_io_send_udp_packet (char *ip, UINT16_t port, const void *p2buffer, int buflen)
{
  ...
  if (p2buffer == NULL)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): Attempted to send NULL p2buffer, drop\n", __func__,
  ...
  if (buflen <= 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): Attempted to send empty p2buffer, drop\n", __func__,
                           rfc4938_config_get_node_id ());
      return (-1);
    }
  ...
  if (dst_addr.sin_addr.s_addr == INADDR_NONE)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, 0.0.0.0 is invalid dst addr.\n", 
                           __func__, rfc4938_config_get_node_id ());
      return (-1);
    }
  ...
  if (dst_addr.sin_port == 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, port has not been established, drop.\n", 
                           __func__, rfc4938_config_get_node_id ());
      return (-1);
    }
  ...
  if (z < 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error sending packet: %s\n", 
                           __func__, rfc4938_config_get_node_id (), 
                           strerror (errno));
    }
  else
    {
      RFC4938_DEBUG_PACKET ("%s:(%u): sent packet len %d to: %s:%d\n", 
                           __func__, rfc4938_config_get_node_id (), 
...
rfc4938_io_listen_for_messages (void)
{
  ...
  if((rfc4938_io_txsock = rfc4938_io_get_udp_socket(rfc4938_config_get_client_port(), NULL)) < 0)
   {
     RFC4938_DEBUG_ERROR ("%s:(%u): could not open general ipc socket\n", __func__,
                          rfc4938_config_get_node_id ());
  ...
  while(1)
    {
      ...
      if(num_fd_ready <= 0)
       {
         RFC4938_DEBUG_ERROR ("%s:(%u): error waiting on select(): %s\n", __func__,
...
rfc4938_io_get_messages (fd_set readable, int num_fd_ready)
{
      ...
      if(rfc4938_io_is_fd_ready(nbr->child_sock, &readable))
        {
          ...
          if (z < 0)
           {
              RFC4938_DEBUG_ERROR ("%s:(%u): error receiving UDP packet: %s\n", __func__,
                                   rfc4938_config_get_node_id (), strerror (errno));
          ...
          {
             RFC4938_DEBUG_PACKET ("%s:(%u): recv UDP packet: (nbr %u, ses %hu), from %s:%hu, len %d\n", __func__,
                                   rfc4938_config_get_node_id (),
                                   nbr->neighbor_id,
                                   nbr->session_id,
  ...
  if(num_fd_ready && rfc4938_io_is_fd_ready(rfc4938_vif_fd, &readable))
    {
      RFC4938_DEBUG_PACKET ("%s:(%u): frame ready on vif\n", __func__, rfc4938_config_get_node_id ());
  ...
  if(num_fd_ready && rfc4938_io_is_fd_ready(rfc4938_eth_dfd, &readable))
    {
      RFC4938_DEBUG_PACKET ("%s:(%u): frame ready on discovery socket\n", __func__, rfc4938_config_get_node_id ());
  ...
  if(num_fd_ready && rfc4938_io_is_fd_ready(rfc4938_eth_sfd, &readable))
    {
      RFC4938_DEBUG_PACKET ("%s:(%u): frame ready on session socket\n", __func__, rfc4938_config_get_node_id ());
...
rfc4938_io_open_vif (const char *ifname, UINT8_t * hwaddr)
{
  ...
  if ((fd = open (path, O_RDWR)) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): error open:%s %s\n", __func__,
  ...
  if (ioctl (fd, TUNSETIFF, &ifr) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): error ioctl(TUNSETIFF):%s %s\n", __func__,
                           rfc4938_config_get_node_id (), path, strerror (errno));
  ...
  if ((sock = socket (AF_INET, SOCK_DGRAM, 0)) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): error socket:%s %s\n", __func__,
                           rfc4938_config_get_node_id (), path, strerror (errno));
  ...
  if (ioctl (sock, SIOCSIFFLAGS, &ifr) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): error ioctl(SIOCSIFFLAGS):%s %s\n", __func__,
                           rfc4938_config_get_node_id (), path, strerror (errno));
  ...
  if (hwaddr)
    {
      if (ioctl (fd, SIOCGIFHWADDR, &ifr) < 0)
        {
          RFC4938_DEBUG_ERROR ("%s:(%u): error ioctl(SIOCGIFHWADDR):%s %s\n", __func__,
                           rfc4938_config_get_node_id (), path, strerror (errno));
...
static int rfc4938_io_read_frame(int fd)
{
  ...
  if(result < 0)
   {
     RFC4938_DEBUG_ERROR ("%s:(%u): error reading frame: %s\n", __func__,
                          rfc4938_config_get_node_id (), strerror (errno));
   ...
   {
     RFC4938_DEBUG_ERROR ("%s:(%u): len %d too short to be ppoe\n", __func__,
     ...
     if((proto == ETH_PPPOE_DISCOVERY) || (proto == ETH_PPPOE_SESSION))
      {
         ...
         RFC4938_DEBUG_PACKET ("%s:(%u): fwd frame to %d children, skipped %d, errors %d\n", 
                              __func__, rfc4938_config_get_node_id (), sent, skip, err);
      }
     else
      {
         RFC4938_DEBUG_PACKET ("%s:(%u): recv non pppoe frame len %d, drop\n", 
                              __func__, rfc4938_config_get_node_id (), result);
      }
   }
...
static int rfc4938_io_send_frame_to_child(UINT16_t session_id, UINT16_t port, void * payload, UINT16_t proto, int payloadlen)
{
  ...
  if (p2buffer == NULL)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): unable to malloc p2buffer\n", 
                           __func__, rfc4938_config_get_node_id());
      return (-1);
    }
  ...
  RFC4938_DEBUG_PACKET ("%s:(%u): send frame to child seqnum %u, payload len %d, msg len %d",
                           __func__, rfc4938_config_get_node_id(), u32seqnum, payloadlen, buflen);
  ...
  if(rfc4938_config_get_vif_mode() == 1)
   {
     ...
     if(write(rfc4938_vif_fd, p2buffer, buflen) < 0)
      {
        RFC4938_DEBUG_ERROR ("%s:(%u): error writting to vif: %s\n", __func__,
                             rfc4938_config_get_node_id (), strerror (errno));
      ...
      {
        RFC4938_DEBUG_PACKET ("%s:(%u): wrote %d bytes to vif\n", __func__,
     ...
     if(proto == ETH_PPPOE_DISCOVERY)
      {
        if(write(rfc4938_eth_dfd, p2buffer, buflen) < 0)
         {
           RFC4938_DEBUG_ERROR ("%s:(%u): error writting to discovery socket: %s\n", __func__,
                                rfc4938_config_get_node_id (), strerror (errno));
         ...
         {
           RFC4938_DEBUG_PACKET ("%s:(%u): wrote %d bytes to discovery socket\n", __func__,
                                rfc4938_config_get_node_id (), buflen);
      ...
      else if(proto == ETH_PPPOE_SESSION)
       {
        if(write(rfc4938_eth_sfd, p2buffer, buflen) < 0)
         {
           RFC4938_DEBUG_ERROR ("%s:(%u): error writting to session socket: %s\n", __func__,
                                rfc4938_config_get_node_id (), strerror (errno));
         ...
         {
           RFC4938_DEBUG_PACKET ("%s:(%u): wrote %d bytes to session socket\n", __func__,
                                rfc4938_config_get_node_id (), buflen);
      ...
      {
         RFC4938_DEBUG_ERROR ("%s:(%u): unsopported proto 0x%hx\n", __func__,
...
rfc4938_io_open_interface (char const *ifname, UINT16_t type, unsigned char *hwaddr)
{
  ...
  if ((fd = socket (domain, stype, htons (type))) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): socket error  %s\n", __func__,
                            rfc4938_config_get_node_id (), strerror (errno));
  ...
  if (setsockopt (fd, SOL_SOCKET, SO_BROADCAST, &optval, sizeof (optval)) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): setsockopt error  %s\n", __func__,
                            rfc4938_config_get_node_id (), strerror (errno));
  ...
  if (hwaddr)
    {
      ...
      if (ioctl (fd, SIOCGIFHWADDR, &ifr) < 0)
        {
           RFC4938_DEBUG_ERROR ("%s:(%u): ioctl error  %s\n", __func__,
                                rfc4938_config_get_node_id (), strerror (errno));
      ...
      if (ifr.ifr_hwaddr.sa_family != ARPHRD_ETHER)
        {
          RFC4938_DEBUG_ERROR ("%s:(%u): error %s\n", __func__,
  ...
  if (ifr.ifr_mtu < PPPOE_ETH_DATA_LEN)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): error MTU %u < %u\n", __func__,
                            rfc4938_config_get_node_id(),  
  ...
  if (bind (fd, (struct sockaddr *) &sa, sizeof (sa)) < 0)
    {
       RFC4938_DEBUG_ERROR ("%s:(%u): bind error  %s\n", __func__,
                            rfc4938_config_get_node_id (), strerror (errno));
...
int rfc4938_io_get_udp_socket(int req_port, int * real_port)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): rx buff len %d bytes\n", __func__,
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): try to bind to %s:%d\n", __func__,
  ...
  if(bind (sock, (struct sockaddr *) &addr, socklen) < 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): unable to bind to address: %s\n", __func__,
                           rfc4938_config_get_node_id (), strerror (errno));
  ...
  if(getsockname (sock, (struct sockaddr *) &addr, &socklen) < 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): unable to get sock info: %s\n", __func__,
                           rfc4938_config_get_node_id (), strerror (errno));
    ...
    {
      RFC4938_DEBUG_EVENT ("%s:(%u): sock info: fd %d, endpoint %s:%hu\n", __func__,
                           rfc4938_config_get_node_id (), 
