...
send_session_packet_to_peer (PPPoEConnection * conn, PPPoEPacket * packet, UINT16_t credits)
{
  ...
  if (p2buffer == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): unable to malloc p2buffer\n", __func__,
  ...
  if ((buflen = rfc4938_ctl_format_child_session_data (u32seqnum, conn->peer_id,
    ...
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): Unable to format message\n", __func__,
                         conn->peer_id, conn->sessionId);
      free (p2buffer);
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sending to peer %u, seqnum %u, payload len %hu, total len %hu\n",
...
send_packet_to_ac (PPPoEConnection * conn, PPPoEPacket * packet, UINT16_t proto)
{
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sending to parent, dst %u, seqnum %u, len %hu, proto 0x%04hx\n",
                     __func__, conn->peer_id, conn->sessionId, conn->peer_id,
...
recv_packet_from_parent (PPPoEConnection * conn, PPPoEPacket * packet)
{
  ...
  if (msglen < 0)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): error:%s\n", __func__,
  ...
  if (hdrchk != HDR_PREFIX)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): len %d, hdrchk 0x%04hx failed test\n", 
                          __func__, conn->peer_id, conn->sessionId, msglen, hdrchk);
  ...
  else
    {
      ...
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): len %d, hdrchk 0x%04hx, parse result %d\n", 
                          __func__, conn->peer_id, conn->sessionId, msglen, hdrchk, result);
...
send_session_start (PPPoEConnection * conn)
{
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sending to peer %u, seqnum %u, pid %hu, credit scalar %hu\n",
                     __func__, conn->peer_id, conn->sessionId, conn->peer_id, u32seqnum, conn->host_id, credit_scalar);
...
send_session_start_ready (PPPoEConnection * conn)
{
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sending to peer %u, seqnum %u, len %d\n", __func__,
...
send_child_ready (PPPoEConnection * conn)
{
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sending to parent, dst %u, seqnum %u, len %d\n", __func__,
...
pppoe_ctl_packet_parser (unsigned char *buff, int msglen, struct sockaddr_in *source_addr, PPPoEPacket * packet)
{
  ...
  if (buff == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s: NULL buff reference\n", __func__);
  ...
  if (msglen <= 0)
    {
      PPPOE_DEBUG_ERROR ("%s: empty buff \n", __func__);
  ...
  if (conn == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s: NULL conn reference\n", __func__);
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): len %d, seqnum %u, from %s:%hu\n", __func__,
                     conn->peer_id, conn->sessionId, 
  ...
  switch (p2ctlmsg->header.u8cmd_code)
    ...
    default:
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu):  unsupported ctrl command 0x%02hhx \n",
                         __func__, conn->peer_id, conn->sessionId, p2ctlmsg->header.u8cmd_code);
    }
...
{
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sending packet len %d\n", __func__,
  ...
  if (z < 0)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): error sending packet len %d:%s:\n", __func__,
...
send_udp_packet (PPPoEConnection * conn, void *p2buffer, int buflen)
{
  ...
  if (conn == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s: NULL connection \n", __func__);
  ...
  if (p2buffer == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s: NULL p2buffer \n", __func__);
  ...
  if (buflen <= 0)
    {
      PPPOE_DEBUG_ERROR ("%s: empty buffer \n", __func__);
...
handle_session_padq (rfc4938_ctl_message_t * p2ctlmsg)
{
  ...
  PPPOE_DEBUG_EVENT ("%s:(%u,%hu): ro: %hhu, rlq: %hhu, res: %hhu, latency: %hu, cdr_scale: %hu, "
...
handle_session_padg (rfc4938_ctl_message_t * p2ctlmsg)
{
  ...
  if((req_credits + conn->peer_credits) > conn->grant_limit)
   {
     if(conn->grant_limit < conn->peer_credits)
      {
         PPPOE_DEBUG_PACKET ("%s:(%u,%hu): req_credits %hu, peer_credits %hu, already at limit, no sending PADG\n", __func__,
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): req_credits %hu, peer_credits %hu, allowed_credits %hu\n", __func__,
  ...
  if (conn->timed_credits)
    {
      ...
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): 1-second grant amount updated by %hu, to %hu\n", __func__,
                         conn->peer_id, conn->sessionId, allowed_credits, conn->grant_limit);
    }
  else
    {
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): send out of band grant %hu\n", __func__,
...
handle_session_start_ready (rfc4938_ctl_message_t * p2ctlmsg)
{
  ...
  PPPOE_DEBUG_EVENT ("%s:(%u,%hu): peer id %u, pid set to %u\n", __func__,
...
handle_session_data_from_peer (rfc4938_ctl_message_t * p2ctlmsg, PPPoEPacket * packet)
{
  ...
  if (conn->sessionId == 0)
    {
      PPPOE_DEBUG_ERROR ("%s: session id has not been established, drop msg\n", __func__);
...
handle_frame_data (rfc4938_ctl_message_t * p2ctlmsg, PPPoEPacket * packet)
{
  ...
  if(proto == ETH_PPPOE_DISCOVERY)
   {
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): recv %d byte discovery msg\n", 
                         __func__, conn->peer_id, conn->sessionId, len);
  ...
  else if (proto == ETH_PPPOE_SESSION)
   {
      if(conn->sessionId == 0)
       {
         PPPOE_DEBUG_ERROR ("%s:(%u,%hu): session not yet established with server, drop msg len %d\n", 
                            __func__, conn->peer_id, conn->sessionId, len);

         return 0;
       }

      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): recv %d byte session msg\n", 
                         __func__, conn->peer_id, conn->sessionId, len);
   ...
   {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): Unsupported proto 0x%04hx\n", 
                         __func__, conn->peer_id, conn->sessionId, proto);
     ...
     if(p2ppp->code == PPP_CONFIG_REQ)
       {
         ...
         if(p2magic != NULL)
           {
             PPPOE_DEBUG_PACKET ("%s:(%u,%hu): acquired peer magic number 0x%08x\n",
                                 __func__, conn->peer_id, conn->sessionId, ntohl(*p2magic));
     ...
     if(p2ppp->code == PPP_CONFIG_REQ)
       {
         ...
         if(p2magic != NULL)
           {
             PPPOE_DEBUG_PACKET ("%s:(%u,%hu): acquired local magic number 0x%08x\n",
                                 __func__, conn->peer_id, conn->sessionId, ntohl(*p2magic));
...
static int handle_lcp_echo_req(PPPoEConnection *conn, PPPoEPacket *packet, PPPHeader *p2ppp)
{
   ...
   PPPOE_DEBUG_PACKET ("%s:(%u,%hu): len %hu, session %hu, code %hhu, id %hhu, opt(s) length %hu, magic 0x%08x, (downstream)\n",
                      __func__, conn->peer_id, conn->sessionId,
                      ntohs(packet->pppoe_length),
   ...
   if(conn->enable_lcp_echo_reply)
       ...
       if(conn->peer_magic != 0)
        {
          ...
          PPPOE_DEBUG_PACKET ("%s:(%u,%hu): set magic number from 0x%08x to 0x%08x, bounce reply back\n",
                              __func__, conn->peer_id, conn->sessionId, ntohl(*p2magic), conn->peer_magic);
        ...
        {
          PPPOE_DEBUG_PACKET ("%s:(%u,%hu): peer magic number is not known, let it thru\n",
                              __func__, conn->peer_id, conn->sessionId);
    ...
    {
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): lcp echo pong disbled,  let it thru\n",
                          __func__, conn->peer_id, conn->sessionId);
...
static UINT32_t * get_lcp_config_magic_number(PPPoEConnection *conn, PPPHeader *p2ppp)
{
   PPPOE_DEBUG_PACKET ("%s:(%u,%hu): code %hhu, id %hhu, opt(s) length %hu\n",
                      __func__, conn->peer_id, conn->sessionId,
   ...
   while(p < (p2ppp->data + ntohs(p2ppp->length) - 4))
    {
      ...
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): option %hhu, option length %hhu\n",
                          __func__, conn->peer_id, conn->sessionId, p2opt->opt, p2opt->length);

      if(p2opt->opt == LCP_MAGIC_NUMBER)
       {
         UINT32_t * p2magic = (UINT32_t *) p2opt->data;

         PPPOE_DEBUG_PACKET ("%s:(%u,%hu): lcp magic number 0x%08x\n",
                             __func__, conn->peer_id, conn->sessionId, ntohl(*p2magic));
...
static int handle_lcp_config_req(PPPoEConnection *conn, PPPoEPacket *packet, PPPHeader *p2ppp)
{
   ...
   PPPOE_DEBUG_PACKET ("%s:(%u,%hu): total len %hu, code %hhu, id %hhu, opt(s) length %hu, opt %hhu, len %hhu\n",
                      __func__, conn->peer_id, conn->sessionId,
                      ntohs(packet->pppoe_length),
   ...
   if(p2magic != NULL)
    {
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): magic number 0x%08x\n",
                      __func__, conn->peer_id, conn->sessionId, ntohl(*p2magic));
...
static int handle_lcp_config_ack(PPPoEConnection *conn, PPPoEPacket *packet, PPPHeader *p2ppp)
{
   PPPOE_DEBUG_PACKET ("%s:(%u,%hu): total len %hu, code %hhu, id %hhu, opt(s) length %hu, consume\n",
                      __func__, conn->peer_id, conn->sessionId,
                      ntohs(packet->pppoe_length),
                      p2ppp->code, p2ppp->id, ntohs (p2ppp->length));
...
static int handle_lcp_config_reject(PPPoEConnection *conn, PPPoEPacket *packet, PPPHeader *p2ppp)
{
   PPPOE_DEBUG_PACKET ("%s:(%u,%hu): total len %hu, code %hhu, id %hhu, opt(s) length %hu\n, consume",
                      __func__, conn->peer_id, conn->sessionId,
                      ntohs(packet->pppoe_length),
                      p2ppp->code, p2ppp->id, ntohs (p2ppp->length));
   ...
   if(p2opt->opt == PPP_IPCP_OPT_IP_ADDRESS)
    {
      ...
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): address %s\n",
                         __func__, conn->peer_id, conn->sessionId, inet_ntoa(*p2addr));
