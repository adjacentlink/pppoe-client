                         ...
                         UINT16_t grant_amount, UINT16_t timed_credits)
{
  ...
  PPPOE_DEBUG_EVENT ("%s:(%u,%hu): Succesfully bound to port %hu\n", __func__,
      ...
      if (conn->peer_pid == 0)
       {
         if(conn->p2p_mode)
          {
            PPPOE_DEBUG_EVENT ("%s:(%u,%hu): Peer pid is 0, we are first up,"
     ...
     else
       {
         ...
         if(conn->p2p_mode)
          {
            PPPOE_DEBUG_EVENT ("%s:(%u,%hu): Peer pid is %u, we are second up,"
  ...
  if (conn->grant_state == PADG_SENT)
    {
      ...
      if ((conn->padg_initial_send_time + MAX_PADC_WAIT_TIME) <= now)
        {
          /* terminate session */
          PPPOE_DEBUG_ERROR ("%s:(%u,%hu): Too many PADCs missed, sent %u PADG's, over interval %ld sec, sending PADT\n",
                             __func__, conn->peer_id, conn->sessionId, conn->padg_tries, now - conn->padg_initial_send_time) ;
      ...
      /* are we within the re-try window */
        ...
        {
         PPPOE_DEBUG_PACKET ("%s:(%u,%hu): waiting for PADC seq 0x%04hx, add %hu credits to cache\n",
                              __func__, conn->peer_id, conn->sessionId, padg_seq_num, credits);
    ...
    {
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): grant %hu, limit %hu, peer_credits %hu, local_credits %hu\n",
...
sendPADG (PPPoEConnection * conn, UINT16_t grant_req)
{
  ...
  if (!conn->sessionId)
   {
     PPPOE_DEBUG_EVENT ("%s:(%u): no session info, not sending PADG\n", __func__,
                        conn->peer_id);
  ...
  if (conn->grant_state == PADG_SENT && grant_req != 0)
    {
      ...
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): grant_state is PADG_SENT, not sending PADG. "
  ...
  if (conn->grant_state == PADC_RECEIVED)
      ...
      if(conn->credit_cache > 0)
       {
         ...
         PPPOE_DEBUG_EVENT ("%s:(%u,%hu): cache_credits %hu, grant_req %hu, current limit %hu", 
                            __func__, conn->peer_id, conn->sessionId, 
  ...
  else
    {
       ...
       */
      PPPOE_DEBUG_EVENT ("%s:(%u,%hu): resending PADG with seq_num 0x%04hx\n", __func__,
                         conn->peer_id, conn->sessionId, padg_seq_num);
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sent PADG fcn:%hu (peer), pending_padc %hu, bcn:%hu (local), seq:0x%04hx, try %u\n",
                      __func__, conn->peer_id, conn->sessionId, fcn, conn->credits_pending_padc, 
...
sendPADC (PPPoEConnection * conn, UINT16_t seq)
{
  ...
  if (!conn->sessionId)
   {
     PPPOE_DEBUG_EVENT ("%s:(%u): no session info, not sending PADC\n", __func__,
                        conn->peer_id);
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sent PADC fcn:%hu (peer), bcn:%hu (local), seq:0x%04hx\n",
                      __func__, conn->peer_id, conn->sessionId, fcn, bcn, seq);
}
...
recvPADG (PPPoEConnection * conn, PPPoEPacket * packet)
{
  ...
  if (findTag (packet, TAG_RFC4938_SEQ_NUM, &sequenceTag) == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): no sequence tag in PADG packet\n", __func__,
                         conn->peer_id, conn->sessionId);
      return;
    }
  ...
  if (findTag (packet, TAG_RFC4938_CREDITS, &creditTag) == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): no credit tag in PADG packet\n", __func__,
                         conn->peer_id, conn->sessionId);
      return;
    }
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): fcn:%hu, bcn:%hu, seq:0x%04hx\n",
                      __func__, conn->peer_id, conn->sessionId, 
                      fcn, bcn, seq);
...
recvPADC (PPPoEConnection * conn, PPPoEPacket * packet)
{
  ...
  if (findTag (packet, TAG_RFC4938_SEQ_NUM, &sequenceTag) == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu)s: no sequence tag in PADC packet, ignore\n", __func__,
                         conn->peer_id, conn->sessionId);
      return;
    }
  ...
  if (findTag (packet, TAG_RFC4938_CREDITS, &creditTag) == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): no credit tag in PADC packet, ignore\n", __func__,
                         conn->peer_id, conn->sessionId);
      return;
    }
  ...
  if (seq == padg_seq_num)
    {
      if(conn->grant_state == PADC_RECEIVED)
       {
         PPPOE_DEBUG_EVENT ("%s:(%u,%hu): ignore duplicate PADC sequence number 0x%04hx",
                            __func__, conn->peer_id, conn->sessionId, seq);
       }
      else
       {
         if(conn->credits_pending_padc > 0)
          {
            PPPOE_DEBUG_EVENT ("%s:(%u,%hu): adding %hu credits_pending_padc to %hu peer_credits",
                               __func__, conn->peer_id, conn->sessionId, conn->credits_pending_padc, conn->peer_credits);
    ...
    {
      PPPOE_DEBUG_EVENT ("%s:(%u,%hu): received PADC with incorrect"
                         " sequence number.  Expected 0x%04hx Received"
                         " 0x%04hx, NOT updating grant_state and padg_retries",
                         __func__, conn->peer_id, conn->sessionId, padg_seq_num, seq);
    }
}
          ...
          UINT8_t resources, UINT8_t rlq, UINT8_t receive_only)
{
  ...
  if (!conn->sessionId)
   {
     PPPOE_DEBUG_EVENT ("%s:(%u): no session info, not sending PADQ\n", __func__,
                        conn->peer_id);
  ...
  if (receive_only > 1)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): receive_only value must be <= 1\n", __func__,
                         conn->peer_id, conn->sessionId);
      return;
    }
  ...
  if (conn->mode != MODE_RFC4938_ONLY)
    {
      if (mdr_scalar > 3)
        {
          PPPOE_DEBUG_ERROR ("%s:(%u,%hu): mdr_scalar value must be <= 3\n", __func__,
                             conn->peer_id, conn->sessionId);
          return;
        }
      if (cdr_scalar > 3)
        {
          PPPOE_DEBUG_ERROR ("%s:(%u,%hu): cdr_scalar value must be <= 3\n", __func__,
                             conn->peer_id, conn->sessionId);
          return;
        }
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): sent PADQ packet with mdr:%hu cdr:%hu latency:%hu\n"
...
recvPADQ (PPPoEConnection * conn, PPPoEPacket * packet)
{
  ...
  if (findTag (packet, TAG_RFC4938_METRICS, &padqTag) == NULL)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): no credit tag in PADQ packet, ignore.\n", __func__,
                         conn->peer_id, conn->sessionId);
      return;
    }
  ...
  if (padqTag.length == 0)
    {
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): received PADQ query packet\n", __func__,
                          conn->peer_id, conn->sessionId);
  ...
  else
    {
      ...
      /* ignore this PADQ */
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): received a PADQ packet from the server, ignore.\n", __func__,
                         conn->peer_id, conn->sessionId);
    }
}
...
sendInBandGrant (PPPoEConnection * conn, PPPoEPacket * packet, UINT16_t credits)
{
  ...
  if (len + TAG_CREDITS_LENGTH + TAG_HDR_SIZE + PPPOE_OVERHEAD > MAX_PPPOE_MTU)
    {
      ...
      PPPOE_DEBUG_PACKET ("%s:(%u,%hu): Request will exceed MTU, NOT adding tag, done\n", __func__,
                          conn->peer_id, conn->sessionId);
  ...
  PPPOE_DEBUG_PACKET
    ("%s:(%u,%hu): peer_credits %hu, consumed_credits %hu, fcn:%hu, bcn:%hu\n", __func__,
...
recvInBandGrant (PPPoEConnection * conn, PPPoEPacket * packet)
{
  ...
  if (tagSize > len)
    {
      PPPOE_DEBUG_ERROR ("%s:(%u,%hu): invalid PPPoE tag length in"
                         " inband grant tag size %hu, tag type %hu, pkt len %hu, ignore.", __func__,
  ...
  PPPOE_DEBUG_PACKET ("%s:(%u,%hu): fcn:%hu, bcn:%hu\n",
                      __func__, conn->peer_id, conn->sessionId, fcn, bcn);
