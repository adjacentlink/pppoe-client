...
inline const char * cmd_code_to_string(UINT8_t code)
 {
    switch(code)
      {
        case CTL_SESSION_START: return "SESSION_START";

        case CTL_SESSION_START_READY: return "SESSION_START_READY";

        case CTL_CHILD_READY: return "CHILD_READY";

        case CTL_CHILD_SESSION_UP: return "CHILD_SESSION_UP";

        case CTL_CHILD_SESSION_TERMINATED: return "CHILD_SESSION_TERMINATED";

        case CTL_CHILD_SESSION_DATA: return "CHILD_SESSION_DATA";

        case CTL_PEER_SESSION_TERMINATED: return "PEER_SESSION_TERMINATED";

        case CTL_PEER_SESSION_DATA: return "PEER_SESSION_DATA";

        case CTL_SESSION_STOP: return "SESSION_STOP";

        case CTL_SESSION_PADQ: return "SESSION_PADQ";

        case CTL_SESSION_PADG: return "SESSION_PADG";

        case CTL_FRAME_DATA: return "FRAME_DATA";

        case CLI_SESSION_INITIATE: return "SESSION_INITIATE";

        case CLI_SESSION_TERMINATE: return "SESSION_TERMINATE";

        case CLI_SESSION_PADQ: return "SESSION_PADQ";

        case CLI_SESSION_PADG: return "SESSION_PADG";

        case CLI_SESSION_SHOW: return "SESSION_SHOW";

        case CLI_SESSION_SHOW_RESPONSE: return "SESSION_SHOW_RESPONSE";

        default: return "UNKNOWN";
     }
}

inline char * ppp_lcp_code_to_string(UINT16_t code)
 {
   switch(code)
    {
      case PPP_CONFIG_REQ:
...
rfc4938_parser_parse_upstream_packet (const void *buff, int bufsize, UINT32_t neighbor_id)
{
  ...
  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
   {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unable to find neighbor_id %u\n",
                           __func__, rfc4938_config_get_node_id (), neighbor_id);

      return;
   }

  RFC4938_DEBUG_EVENT ("%s:(%u): len %u, src %u\n", __func__,
...
{
  RFC4938_DEBUG_EVENT ("%s:(%u): len %u, nbr %u\n", __func__,
...
rfc4938_parser_parse_packet_i(const void *buff, int bufsize, rfc4938_neighbor_element_t *nbr, int dir)
{
  ...
  if (buff == NULL)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, NULL buff\n", __func__,
  ...
  if (bufsize < (int) sizeof (rfc4938_ctl_message_header_t))
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, msg too short (%d < %zd)\n",
                           __func__, rfc4938_config_get_node_id (), bufsize,
  ...
  RFC4938_DEBUG_PACKET ("%s:(%u): recv msg %s, len %d, seqnum %u, child_pid %u\n",
      ...
      if((seq != nbr->last_seqnum + 1) && nbr->child_port)
        {
          ...
          RFC4938_DEBUG_PACKET ("%s:(%u): missed seqnum %u, last %u, total_missed %u, child_pid %u\n",
                                __func__, rfc4938_config_get_node_id (),
  ...
  switch (p2ctlmsg->header.u8cmd_code)
    ...
    default:
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unknown command 0x%02hhx \n",
                           __func__, rfc4938_config_get_node_id (), p2ctlmsg->header.u8cmd_code);
      break;
    }
                                   ...
                                   UINT16_t max_data_rate)
{
  ...
  if (nbr->nbr_session_state != ACTIVE)
    {
      RFC4938_DEBUG_EVENT ("%s:(%u): neighbor_id %u is not active yet, drop\n", __func__,
                           rfc4938_config_get_node_id (), neighbor_id);
  ...
  if (p2buffer == NULL)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unable to allocate buffer for ctl_padq_request\n",
                           __func__, rfc4938_config_get_node_id ());
  ...
  if ((buflen = rfc4938_ctl_format_session_padq (u32seqnum++,
    ...
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unable to format message\n",
                           __func__, rfc4938_config_get_node_id ());
  ...
  if(retval == buflen)
    {
      RFC4938_DEBUG_EVENT ("%s:(%u): sent PADQ for neighbor %u, seqnum %u, "
    ...
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): failed to send PADQ for neighbor %u, "
                           "ro %hhu, rlq %hhu, resources %hhu, latency %hu, "
                           "cdr_scale %hu, cdr %hu, mdr_scale %hu, mdr %hu\n",
                            __func__, rfc4938_config_get_node_id (),
...
void rfc4938_parse_ppp_packet(const PPPoEPacket *packet, int framelen, const char * direction)
{
  ...
  if(proto == ETH_PPPOE_DISCOVERY)
    {
       RFC4938_DEBUG_PACKET ("%s: flen %d, type %hhu, ver %hhu, code %hhu (%s), session %hu, (%s)\n", __func__, 
                             framelen,
                             packet->pppoe_type, 
                             packet->pppoe_ver, 
                             packet->pppoe_code, 
  ...
  else if(proto == ETH_PPPOE_SESSION)
    {
       ...
       switch(ntohs(tag->type))
        {
          ...
          case TAG_RFC4938_SCALAR:
            ...
            RFC4938_DEBUG_PACKET ("%s: found tag 0x%04hx, skip %d bytes\n", __func__, ntohs(tag->type), 
         ...
         if(p2ppp->code == PPP_ECHO_REQ || p2ppp->code == PPP_ECHO_REPLY)
           {
             ...
             RFC4938_DEBUG_PACKET ("%s: flen %hu, type 0x%04hx (PPP_LCP), code %hhu (%s), session %hu, id %hhu, magic 0x%08x, (%s)\n", 
                                   __func__, 
                                   framelen,
                                   type, 
                                   p2ppp->code, 
           ...
           {
             RFC4938_DEBUG_PACKET ("%s: flen %hu, type 0x%04hx (PPP_LCP), code %hhu (%s), session %hu, id %hhu (%s)\n", __func__, 
                                   framelen, 
                                   type, 
                                   p2ppp->code, 
                                   ppp_lcp_code_to_string(p2ppp->code), 
       ...
       {
         RFC4938_DEBUG_PACKET ("%s: flen %hu, type 0x%04hx (PPP_IPCP), code %hhu (%s), session %hu, id %hhu (%s)\n", __func__, 
                               framelen, 
                               type, 
                               p2ppp->code, 
                               ppp_lcp_code_to_string(p2ppp->code), 
...
rfc4938_parser_cli_padg_session_i (UINT32_t neighbor_id, UINT16_t credits)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): received CLI request to send %hu credits for nbr %u\n", 
                       __func__, rfc4938_config_get_node_id (), credits, neighbor_id);
  ...
  if (p2buffer == NULL)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unable to allocate buffer for ctl_padg_request\n",
                           __func__, rfc4938_config_get_node_id ());
  ...
  if(retval == buflen)
    {
      RFC4938_DEBUG_PACKET ("%s:(%u): sent PADG seqnum %u, with %hu credits for neighbor %u\n",
                           __func__, rfc4938_config_get_node_id (), u32seqnum - 1, credits, nbr->neighbor_id);
    }
  else
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): failed to send PADG with %hu credits for neighbor %u\n",
                           __func__, rfc4938_config_get_node_id (), credits, nbr->neighbor_id);
    }
...
rfc4938_parser_cli_show_session_i (void)
{
  ...
  if (z < 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error sending message:", __func__,
                           rfc4938_config_get_node_id ());
    ...
    {
      RFC4938_DEBUG_PACKET ("%s:(%u): sent message to %s:%hu, len %d", __func__,
...
rfc4938_parser_ctl_recv_session_start_i (UINT32_t neighbor_id, UINT32_t pid, UINT16_t scalar)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): nbr %u, pid %u, scalar %u\n", __func__,
                       rfc4938_config_get_node_id (), neighbor_id, pid, scalar);

  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unknown nbr %u, drop\n", __func__,
                           rfc4938_config_get_node_id (), neighbor_id);
  ...
  if (nbr->nbr_session_state == ACTIVE || nbr->child_pid != 0)
    {
      ...
      if (nbr->nbr_session_state == INACTIVE)
        {
          RFC4938_DEBUG_EVENT ("%s:(%u): pppoe processs for nbr %u is inactive"
                               " saving nbr pid %u \n", __func__, rfc4938_config_get_node_id (),
                               neighbor_id, pid);
      ...
      else
        {
          ...
          if (p2buffer == NULL)
            {
              RFC4938_DEBUG_ERROR ("%s:(%u): error, unable to allocate buffer for ctl_start_ready\n", 
                                  __func__, rfc4938_config_get_node_id ());
          ...
          if ((buflen = rfc4938_ctl_format_session_start_ready (u32seqnum++, 
            ...
            {
              RFC4938_DEBUG_ERROR ("%s:(%u): Unable to format message\n", __func__,
                                   rfc4938_config_get_node_id ());
            ...
            {
              RFC4938_DEBUG_EVENT ("%s:(%u): pppoe processs for nbr %u is active"
    ...
    {
      RFC4938_DEBUG_EVENT ("%s:(%u): pppoe process for nbr %u, is not active, initiating now.\n",
                           __func__, rfc4938_config_get_node_id (), neighbor_id);
...
rfc4938_parser_ctl_recv_child_ready_i (UINT32_t neighbor_id, UINT16_t port, UINT32_t pid)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): nbr %u, port %hu, pid %u\n", 
                       __func__, rfc4938_config_get_node_id(),
                       neighbor_id, port, pid);

  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unknown neighbor\n", __func__,
                           rfc4938_config_get_node_id());
    }
  else if (nbr->child_pid != pid)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, child_pid mismatch expected %u, got %u\n", __func__,
...
rfc4938_parser_ctl_recv_child_session_terminated_i (UINT32_t neighbor_id, UINT16_t session_id)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): session %hu, neighbor %u\n", __func__,
                       rfc4938_config_get_node_id (), session_id, neighbor_id);

  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unknown nbr %u\n", __func__,
                           rfc4938_config_get_node_id (), neighbor_id);
    }
  else
    {
...
static void rfc4938_parser_ctl_recv_peer_session_data_i(UINT32_t neighbor_id, const void *buff, const int bufsize)
{
  ...
  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unknown nbr %hu\n", __func__,
                           rfc4938_config_get_node_id (), neighbor_id);
    }
  else
    {
      RFC4938_DEBUG_EVENT ("%s:(%u): nbr %u, len %d, session state is %s\n",
                           __func__, rfc4938_config_get_node_id (), 
                           neighbor_id, bufsize,
                                                         ...
                                                         const void *buff, const int bufsize)
{
  ...
  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
  ...
  else
    {
      ...
      RFC4938_DEBUG_EVENT ("%s:(%u): neighbor %u, len %d, seqnum %u\n", __func__,
...
rfc4938_parser_ctl_recv_child_session_up_i (UINT32_t neighbor_id, UINT16_t session_id, UINT32_t pid)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): nbr %u, session id %hu, pid %u\n", 
                       __func__, rfc4938_config_get_node_id (),
  ...
  if (rfc4938_neighbor_pointer_by_nbr_id (neighbor_id, &nbr) != 0)
    {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, unknown neighbor %u\n", __func__,
                           rfc4938_config_get_node_id (), neighbor_id);
  ...
  if(session_id != 0)
    {
      ...
      RFC4938_DEBUG_EVENT ("%s:(%u): nbr %u, session id %hu, pid %u set to ACTIVE\n", 
                       __func__, rfc4938_config_get_node_id (),
                       neighbor_id, session_id, pid);
    }
  else
   {
      RFC4938_DEBUG_ERROR ("%s:(%u): error, neighbor_id %u already on session %hu\n", __func__,
...
{
  RFC4938_DEBUG_EVENT ("%s:(%u): initiating session for nbr %u, credit_scalar %hu\n",
                       __func__, rfc4938_config_get_node_id (), neighbor_id, credit_scalar);
...
rfc4938_parser_cli_terminate_session_i (UINT32_t neighbor_id, UINT16_t cmdSRC)
{
  ...
  RFC4938_DEBUG_EVENT ("%s:(%u): neighbor_id %u, cmd %hu\n",
                       __func__, rfc4938_config_get_node_id (), neighbor_id, cmdSRC);
