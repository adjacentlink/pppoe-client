...
#define ARGC_MAX   ( 5 )
  ...
  if (!fp)
    {
      RFC4938_DEBUG_ERROR ("%s: error, problem opening config file: %s\n",
                           __func__, filename);
      return (-1);
    }
  ...
  while (fgets (input_string, MAX_INPUT_LENGTH, fp))
    {
      ...
      /* credit threshold */
      else if (strncmp (argv[0], "CREDIT_THRESHOLD", strlen ("CREDIT_THRESHOLD")) == 0)
        {
          ...
          if(CONFIG.credit_threshold < 0.0f || CONFIG.credit_threshold > 1.0f)
            {
              RFC4938_DEBUG_ERROR ("%s: CREDIT_THRESHOLD of %f is invalid, must be [0.0 - 1.0]\n",
                                    __func__, CONFIG.credit_threshold);
              return (-1);
            }
        }
      ...
      /* session timeout */
        ...
        {
          RFC4938_DEBUG_ERROR ("%s: Unknown config file item (%s) \n", __func__, argv[0]);
          return (-1);
        }
    }
  ...
  if(CONFIG.sinr_max <= CONFIG.sinr_min)
   {
     RFC4938_DEBUG_ERROR ("%s: sinr max %f, must be larger than sinr min %f \n", 
                          __func__, CONFIG.sinr_max, CONFIG.sinr_min);
