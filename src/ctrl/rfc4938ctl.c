...
rfc4938ctl_send_show_message (void)
{
    ...
    if (p2buffer == NULL)
    {
        LOGGER(LOG_ERR, "ERROR: SIZEOF_CLI_SHOW_REQUEST malloc failed\n");
    ...
    {
        LOGGER(LOG_ERR, "ERROR: rfc4938_cli_format_session_show failed\n");
    ...
    {
        LOGGER(LOG_ERR, "ERROR: rfc4938ctl_send_message failed\n");
                              ...
                              UINT8_t resources, UINT8_t rlq, UINT8_t receive_only)
{
    ...
    if (receive_only == 0)
    {
        LOGGER(LOG_INFO, "rfc4938ctl_send_padq_message neighbor %u, max-data-rate %hu %hu, cur-data-rate"
    ...
    {
        LOGGER(LOG_ERR, "ERROR: unknown error in rfc4938ctl_send_padq_message function\n");
    ...
    {
        LOGGER(LOG_ERR, "ERROR: rfc4938_cli_format_session_padq failed\n");
...
rfc4938ctl_send_padg_message (UINT32_t neighbor_id, UINT16_t credits)
{
    ...
    LOGGER(LOG_INFO, "rfc4938ctl_send_padg_message neighbor %u credits %hu\n", neighbor_id,
                         credits);
    if ((buflen =
                rfc4938_cli_format_session_padg (u32seqnum++, neighbor_id, credits, p2buffer)) != SIZEOF_CLI_PADG_REQUEST)
    {
        LOGGER(LOG_ERR, "ERROR: rfc4938_cli_format_session_padg failed\n");
...
rfc4938ctl_send_initiate_message (int cmd, UINT32_t neighbor_id, UINT16_t credit_scalar)
{
    ...
    if (cmd == 0)
    {
        LOGGER(LOG_INFO, "rfc4938ctl_send_initiate_message all scalar %hu\n", credit_scalar);
        neighbor_id = 0;          /* '0' signifies 'all' neighbors */
    }
    else if (cmd == 1)
    {
        LOGGER(LOG_INFO, "rfc4938ctl_send_initiate_message neighbor %u, scalar %hu\n",
                             neighbor_id, credit_scalar);

    }
    else
    {
        LOGGER(LOG_ERR, "ERROR: unknown error in rfc4938ctl_send_initiate_message function\n");
                  ...
                  p2buffer)) != SIZEOF_CLI_INITIATE_REQUEST)
    {
        ...
        LOGGER(LOG_ERR, "ERROR: rfc4938_cli_format_session_initiate failed\n");
...
rfc4938ctl_send_terminate_message (int cmd, UINT32_t neighbor_id)
{
    ...
    if (cmd == 0)
    {
        LOGGER(LOG_INFO, "terminate all\n");
        neighbor_id = 0;          /* '0' signifies 'all' neighbors */
    }
    else if (cmd == 1)
    {
        LOGGER(LOG_INFO, "termiante neighbor %u\n", neighbor_id);
    }
    else
    {
        LOGGER(LOG_ERR, "ERROR: unknown error in terminate function\n");
    ...
    {
        LOGGER(LOG_ERR, "ERROR: rfc4938_cli_format_session_terminate failed\n");
...
#define DGRAM_LEN    ( 2048 )
    ...
    if (s == -1)
    {
        LOGGER(LOG_ERR, "Unable to create socket\n");
        return (-1);
    }
...
main (int argc, char *argv[])
{
    ...
    if (argv[CMD] == NULL)
    {
        LOGGER(LOG_ERR, "ERROR: Must enter command\n%s\n", rfc4938ctl_usage);
        return (-1);
    }
    ...
    else if (!strncmp (argv[CMD], "padq", strlen ("padq")))
    {
        ...
        {
            LOGGER(LOG_ERR, "ERROR: Invalid Command:\npadq: all arguments" " not included\n");
            return (-1);
        }
    ...
    else if (!strncmp (argv[CMD], "terminate", strlen ("terminate")))
    {
        ...
        {
            LOGGER(LOG_ERR, "ERROR: Invalid Command: terminate\n");
            return (-1);
        }
    }
